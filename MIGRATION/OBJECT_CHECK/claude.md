...
- **STEP 3/4** — 개수 집계 6종 INSERT (OBJECT_NAME = CONSTRAINT / TABLE / TABLE PARITTION /
  INDEX / INDEX PARITTION / OTHERS OBJECT). 검증: **OWNER별**로 `MIG_OBJ_CNT_{ASIS|TOBE}` 를
  조회하여, 각 OWNER 마다 6종 OBJECT_NAME 의 `cnt` 와 소유자 합계를 출력한다(해당 OWNER 에
  없는 종류는 `cnt=0`). `MIG_OBJ_CNT` 조회 자체가 오류일 때만 `FAIL` 로 중단. ASIS/TOBE 는
  OWNER 명이 다를 수 있어(EBDBA↔GEBOWN) 완료 현황은 각 DB 실제 OWNER 기준이며, OWNER 매핑
  비교는 STEP 5(GAP)에서 한다.

구현: `verify_dict()`/`verify_count()` 가 **대상마다 개별 `SELECT COUNT(*)` 를 셸 루프로
실행**하여 `[1/7] [2/7] …` 처럼 한 줄씩 순차 출력하고, 항목 사이에 `sleep ${STEP_SLEEP}`
(기본 `0.5` 초) 를 둔다(진행 상황이 눈에 보이도록). 스칼라가 숫자로 안 나오면 `FAIL` 로 표기하고
그 STEP 을 중단한다. **단, STEP 1/2 에서 CTAS 로 테이블이 생성되지 않아 조회가 `ORA-00942`
(table or view does not exist) 로 떨어지는 경우는 `FAIL` 이 아니라 `OK rows=0 (table not found)`
로 예외 처리**하여 파이프라인을 중단하지 않는다(원본 딕셔너리에 해당 오브젝트가 없어 미생성된
정상 상황을 허용). `OBJECT_NAME` 라벨은 `sql/*.sql` 의 리터럴
...
**캐노니컬 `sql/*.sql` 에는 DB LINK 를 박아두지 않는다.** ASIS 원격 딕셔너리 뷰는 링크 없이
(`FROM DBA_TABLES`) 두고, 셸이 실행 시점에 `${BASE_PATH}/tmp/` 사본에서 각 원격 뷰에 선택한
DB LINK 를 붙여(`DBA_TABLES` → `DBA_TABLES@<DBLINK>`) 그 tmp 파일을 `sqlplus` 로 실행한다.
원본 SQL 은 그대로 둔다. 주입 대상 뷰 목록은 `object_check.sh` 의 `OC_REMOTE_VIEWS`
(`DBA_CONSTRAINTS DBA_TABLES DBA_TAB_COLUMNS DBA_INDEXES DBA_TAB_PARTITIONS DBA_IND_PARTITIONS
DBA_OBJECTS`)이며, 단어 경계(`\b`) 매칭이라 로컬 복제본 `DBADM.DBA_ASIS_*` 에는 붙지 않는다.
**DB LINK 주입은 1단계(link)에서만** 이뤄진다. 3단계는 로컬 복제본만 조회하므로 nolink 이며
DB LINK 를 붙이지 않는다(3단계 SQL 의 `DBA_OBJECTS` 는 로컬 참조). 원격 뷰 이름을 SQL 에서
바꾸면 `OC_REMOTE_VIEWS` 도 함께 수정한다.
